...
export class DetalleMomentosDeVidaComponent implements OnInit, OnDestroy {
  ...
  getLoMasConsultado() {
    this.getUbicacion = this.serviceGeoView.getUbicacion$.subscribe((data: any) => {
      if (data.codigoMunicipio != "TodosLosMunicipios" && data.codigoMunicipio != '') {
        this.serviceFichaTramite.getLoMasConsultadoPorUbicacion(data.codigoMunicipio, this.id_momento);
  ...
  navigateSeccion() {
    this.getUbicacion = this.serviceFichaTramite.getMasConsultado$.subscribe((data: any) => {
      if (data.data.length > 0) {
        this.estado_mas_consultado = false;
        this.router.navigate(['/categorias-subcategorias-pwa/' + this.id_momento + '/lo-mas-consultado']);
      } else {
        this.estado_mas_consultado = true;
        this.router.navigate(['/categorias-subcategorias-pwa/' + this.id_momento + '/tramites-destacados']);
      }
    })
  }
